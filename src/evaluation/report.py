...
from src.utils.paths import DOCS_DIR, RESULTS_DIR, resolve
...
def build_report(
    ...
    retriever_summary = metric_summary(retrieval_df)
    best_per_dataset = best_rows(retrieval_df)
    sweep_top = sweep_summary(sweep_df)
    rag_table = rag_summary(rag_records)
    ...
    return "\n".join([
        ...
        "",
        dataframe_to_markdown(retriever_summary),
        "",
...
def generate_report(paths: ReportPaths) -> Path:
    ...
    paths.report_md.write_text(report, encoding="utf-8-sig")
