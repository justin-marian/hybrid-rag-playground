...
- Cached under `data/cache/embeddings/<model>_norm1.npz` as a single compressed
  numpy archive (`keys`, `vecs`).
- Cache is keyed by `sha1(text)`. Identical chunk text across datasets shares
...
- Object UUIDs in Weaviate are deterministic (`uuid5(NAMESPACE_URL, chunk_id)`),
  so re-running `run_indexing` overwrites instead of duplicating.
- `chunk_id` follows the `<doc_id>::chunk_<index>` convention so we can collapse
  back to `doc_id` for IR metrics.

## Metric correctness

- IR metrics (Recall@10, MRR, nDCG@10) operate on **doc-level** rankings.
  Chunks are deduplicated to their `doc_id` *before* scoring, so two chunks
  from the same paper count once.
